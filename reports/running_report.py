...
Usage:
    ...
    python3 reports/running_report.py --db path/to/your.sqlite --out-dir out/
"""
...
DEFAULT_DB = Path(__file__).resolve().parent.parent / "example-data" / "cordelia-sample-running.sqlite"
...
def load_sessions(conn: sqlite3.Connection) -> pd.DataFrame:
    return pd.read_sql_query(
        """
        SELECT file_number, start_time, total_distance, total_timer_time, avg_heart_rate
        FROM Session
        ORDER BY start_time
        """,
        conn,
...
def plot_pace_trend(sessions: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.invert_yaxis()  # faster runs (lower pace) plot higher, reads as "improvement"
    fig.autofmt_xdate()
    fig.tight_layout()
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_DB,
        help="Path to a Cordelia SQLite database (default: the bundled running example)",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "output" / "running",
    ...
    try:
        ...
        plot_route_map(track, args.out_dir / "route_map.png", f"Route — {first_run['start_time'].date()}")
    finally:
        conn.close()
