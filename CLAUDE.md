# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working in this repository.

## What this repo is

`antonio` is a small, public set of Python example scripts ("seed" examples) showing how to query
a **Cordelia** SQLite database and produce basic report plots. It is not a maintained analytics
tool. Read `docs/objectives-and-requirements.md` for the objectives and explicit non-goals, and
`docs/plan.md` for the phased scope of work, the exact schema detail, and the encoding gotchas
below — both docs are the source of truth for scope decisions, not this file.

**Namesake:** Antonio, the sea captain in *Twelfth Night* who stakes Sebastian with support to get
going, then lets him make his own way — the intended relationship between this repo and its users.
It's one repo in a larger Shakespeare-named family (see the sibling `cordelia`, `goneril`, etc.
repos), but unlike the rest of that family this one is genuinely public on GitHub, not a private
Fossil repo with just a public overview page.

## Version control

Plain **git** on GitHub (`venholaconsultinginc/antonio`), not Fossil. This is the one repo in the
family that isn't Fossil-hosted — don't apply the sibling repos' Fossil conventions here.

## Data source: Cordelia's SQLite schema

Cordelia (the sibling `../cordelia` C++ project) parses Garmin `.FIT` files into ~120 SQLite
tables, one per FIT message type. This repo only ever touches nine of them — `FileID`, `Activity`,
`Event`, `DeviceInfo`, `Session`, `Lap`, `Record`, `FileCreator`, `Sport` — the set the running,
cycling, and kayaking generators between them populate (a plain running activity only needs the
first seven; `FileCreator`/`Sport` were added for cycling, see `docs/plan.md`'s Phase 2b). Full
column-level detail is in `docs/plan.md`; if you're touching `cordelia_db.py` or a generator, read
that first.

**Before trusting any schema detail here or in `docs/plan.md`: it was captured from `cordelia` on
2026-08-05 and is not guaranteed current.** If you're changing the generator or helper module,
diff the relevant `CREATE TABLE` statements in `../cordelia/src/records/*.cpp` against what's
copied here first — `cordelia` is under active development and this repo does not track it
automatically.

Three encoding rules matter more than anything else in this codebase, because getting them wrong
produces a database that opens fine but is silently corrupt:

1. **GPS is semicircle-encoded**, not plain degrees: `position_lat`/`position_long` are `INTEGER`
   int32, `semicircles = round(degrees * 2^31 / 180)`.
2. **Every timestamp-semantic column is ISO 8601 TEXT**, fixed 19-char
   `YYYY-MM-DDTHH:MM:SS` (UTC, no offset) — including `Session.start_time` and `Lap.start_time`,
   which is worth calling out only because it wasn't always true. Both were declared/actual-type
   mismatches (`Session.start_time` said `INTEGER` but held ISO 8601 text; `Lap.start_time` said
   `TEXT` but held a raw Garmin-epoch integer) discovered while building this repo — see cordelia
   ticket #72, fixed 2026-08-06 (fossil commit `b2a77bad4b`), which standardized every
   timestamp-semantic column in `cordelia`'s schema on `ISO8601DateTime`/`TEXT`. There is no
   Garmin-epoch-integer encoding left anywhere in the tables this repo touches. The lesson still
   stands for any *new* column this repo starts populating: don't assume a `CREATE TABLE` type
   is a reliable guide without checking the real bind/extract call, or real data — that class of
   drift is always possible elsewhere in `cordelia`'s ~120 tables.
3. `FileID` requires a distinct `(serial_number, time_created)` pair per activity, and `Session`/
   `Lap`/`Activity` each have their own `event_id`/`event_type` pair, distinct from the `Event`
   table's generic timer values (`event_id=0`, `event_type` 0/4 for start/stop_all). Confirmed
   against real data: `Session.event_id=8`, `Lap.event_id=9`, `Activity.event_id=26`, all with
   `event_type=1` — don't reuse the `Event` table's timer constants for these three tables' own
   fields.

All conversions belong in `cordelia_db.py`, not reimplemented per script.

## Style

This is example/seed code meant to be read and copied by someone learning the schema, not a
library. Favor clarity over abstraction: prefer three similar, obvious scripts over one clever
parameterized one. Every report script should be readable top-to-bottom on its own.

## Data hygiene

Never commit a real Cordelia export or any file containing real personal fitness data — only
synthetic data generated by `generate_sample_data.py`/`generate_sample_data_bike.py`/
`generate_sample_data_kayak.py`. All three generators write a real SQLite database directly (see
"Running" below); no generated `.db`/`.sqlite` file is ever committed, regenerate locally instead.
Real reference `.fit`/`.sqlite` pairs were used locally, read-only, to ground-truth the cycling and
kayaking generators' schema fidelity (see `data/SOURCES.md`) — those pairs live under `/routes/`,
which is gitignored specifically so they can never be swept into a commit. **One deliberate
exception**: `data/route_kayak.gpx` is a geometry-only extraction (position points only, no
personal data) from the kayak reference, committed with the owner's explicit confirmation — see
`data/SOURCES.md`'s `route_kayak.gpx` section for exactly what was and wasn't taken.

## Running

```bash
python3 generate_sample_data.py                 # writes output/sample_data.sqlite, stdlib only
python3 generate_sample_data_bike.py             # writes output/sample_data_bike.sqlite, stdlib only
python3 generate_sample_data_kayak.py            # writes output/sample_data_kayak.sqlite, stdlib only
```

Each generator opens its own fresh `.sqlite` file directly (via `cordelia_db.py`'s
`create_schema`/`insert_row`/`insert_many` helpers) — no separate `sqlite3 ... <` step, and no
intermediate `.sql` text file. Revised 2026-08-06; see `docs/plan.md`'s Phase 2/2b correction
notes for why (the previous approach hand-tracked `file_number` as a counter rather than reading
back SQLite's own `AUTOINCREMENT` assignment).

Report/plotting scripts (Phase 3 in `docs/plan.md`) don't exist yet — `requirements.txt`
(pandas, matplotlib) is a forward declaration for them, not needed by either generator itself.

No test suite — validation is running the pipeline end to end and checking the output looks sane
(closed-loop route, pace/HR/temperature within their +/-10% bands, distance matching the route
length, GPS bounding box matching the real-world location, `Session.start_time`/`Lap.start_time`
agreeing with `Timestamp`), per `docs/plan.md`'s Phase 5. For the cycling generator specifically,
also cross-check against the real reference database's actual column population before trusting
a new field — see Phase 2b's writeup for what's already been verified.
