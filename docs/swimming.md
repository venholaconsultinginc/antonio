...
length, 1,066 of them across the 26 example sessions. Each row carries that length's own
`total_timer_time`, `total_strokes`, `avg_speed`, `avg_swimming_cadence` and a `swim_stroke` code.
`Length` sits below `Lap` (4 per session here) in the same hierarchy the other sports use, so the
finest grain available is a single 25-metre length rather than a whole swim.

`Session` adds the swim-specific summary fields (`pool_length`, `number_lengths`, `total_strokes`,
`avg_stroke_count` and `avg_stroke_distance`), so most session-level questions never need the
`Length` table at all.
...
from running, cycling and kayaking. The `cadence` field is empty too, since swimming cadence lives
in `Length.avg_swimming_cadence` instead.
...
And a SWOLF distribution built from the `Length` table, one observation per pool length rather than
per session:
...
better. Plot it alongside pace, because it catches what pace alone hides: you can always swim a
length faster by thrashing, but that costs strokes, so the score only improves when speed and
...
Plots go to `reports/output/swimming/` by default; pass `--out-dir` to write them somewhere else.
